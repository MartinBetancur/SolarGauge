...
const CreateOfferForm = ({ apiUrl, userId, onCreateOffer }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        console.log(userId);
        const offerData = { ...formData, seller_id: userId }; // Agregar userId como seller_id
        try {
            const response = await axios.post(`${apiUrl}/api/v1/offers/`, offerData);
            onCreateOffer(response.data);
            setFormData({
                energy_amount: 0,
                price_per_unit: 0,
                offer_type: "buy",
